...
export default function UniversityProfileScreen() {
  ...
  const followerCount = Number(university?.followers_count || 0);
  const subCount = Number(university?.child_count || 0);
    ...
    apiClient
      ...
      .then((resp) => {
        ...
        if ((resp.data as any)?.success) {
          setUniversity((resp.data as any)?.university || null);
        } else {
          setError((resp.data as any)?.error || 'Unable to load university.');
        }
      })
      .catch(() => {
        if (!mounted) return;
        setError('Unable to load university.');
      })
      .finally(() => {
        if (!mounted) return;
        setIsLoading(false);
  ...
  const bannerSrc = buildUploadSrc(
    university?.banner_path || '/uploads/banners/DefaultBanner.jpeg'
  );
  const logoSrc = buildUploadSrc(
    university?.logo_path || '/uploads/logos/School Image.png'
  );
  ...
  if (!university) {
    return (
      <AppShell>
        <View style={styles.loadingWrap}>
          <ThemedText style={styles.errorText}>University not found.</ThemedText>
        </View>
      </AppShell>
    );
  }
    ...
    <AppShell>
      <ScrollView contentContainerStyle={styles.container}>
        <View
          style={[
            styles.heroCard,
            isNarrow && styles.heroCardNarrow,
          ]}
        >
          ...
          >
            <View style={styles.heroLeft}>
              <View style={styles.heroText}>
                <ThemedText style={styles.heroTitle}>{university.name || 'University'}</ThemedText>
                {university.tagline ? (
                  <ThemedText style={styles.heroSub}>{university.tagline}</ThemedText>
                ) : null}
                {university.location ? (
                  <ThemedText style={styles.heroSub}>{university.location}</ThemedText>
                ) : null}
                <View style={styles.heroRow}>
                  ...
                  <ThemedText style={styles.heroMeta}>
                    {subCount} {subCount === 1 ? 'sub-community' : 'sub-communities'}
                  </ThemedText>
                </View>
              </View>
            </View>
            <View style={styles.heroActions}>
              <Pressable
                ...
                style={[styles.followButton, university.is_following && styles.followButtonSecondary]}
              >
                <LinearGradient
                  colors={
                    university.is_following
                ...
                >
                  <ThemedText
                    style={[
                      ...
                      university.is_following && styles.followButtonTextSecondary,
                    ]}
                  >
                    {isFollowBusy ? 'Updating…' : university.is_following ? 'Unfollow' : 'Follow'}
                  </ThemedText>
                </LinearGradient>
              </Pressable>
              {canEditCommunity ? (
          ...
          <View style={styles.contentCard}>
            <View style={styles.sectionHeader}>
              ...
              <ThemedText style={styles.sectionSub}>
                Ambassador-picked threads and forums for {university.name}.
              </ThemedText>
            </View>
              ...
              pinnedItems.map((item) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => {
                      if (isThread && item.thread_id) {
                        router.push(`/thread/${item.thread_id}`);
                      } else if (!isThread && item.forum_id) {
                        router.push(`/info/forum/${item.forum_id}`);
                      }
                    }}
                  >
        ...
        {activeTab === 'subgroups' ? (
          <View style={styles.contentCard}>
            <View style={styles.sectionHeader}>
              ...
              <ThemedText style={styles.sectionSub}>
                Departments and programs connected to {university.name}.
              </ThemedText>
            </View>
            {loadingSub ? <ActivityIndicator /> : null}
            {!loadingSub && subcommunities.length === 0 ? (
              <ThemedText style={styles.mutedText}>No sub-communities yet.</ThemedText>
            ) : (
              subcommunities.map((sub) => (
                ...
                >
                  ...
                  <View style={styles.subContent}>
                    ...
                    {sub.tagline ? (
                      <ThemedText style={styles.subMeta}>{sub.tagline}</ThemedText>
                    ) : null}
                    {sub.location ? (
                      <ThemedText style={styles.subMeta}>{sub.location}</ThemedText>
                    ) : null}
        ...
        {activeTab === 'posts' ? (
          <View style={styles.contentCard}>
            <View style={styles.sectionHeader}>
              ...
              <ThemedText style={styles.sectionSub}>
                The latest forum threads highlighted by ambassadors.
              </ThemedText>
            </View>
            {loadingPinned ? <ActivityIndicator /> : null}
            {!loadingPinned && pinnedItems.length === 0 ? (
              ...
              pinnedItems.map((item) => (
                <Pressable
                  ...
                  style={styles.pinnedCard}
                  onPress={() => {
                    if (item.item_type === 'thread' && item.thread_id) {
                      router.push(`/thread/${item.thread_id}`);
        ...
        {activeTab === 'qa' ? (
          <View style={styles.contentCard}>
            <View style={styles.sectionHeader}>
              <ThemedText style={styles.sectionTitle}>Q+A</ThemedText>
              <ThemedText style={styles.sectionSub}>
                Ask questions and see responses from ambassadors.
              </ThemedText>
            </View>
                ...
                <View key={q.question_id} style={styles.qaCard}>
                  <View style={styles.qaHeader}>
                    ...
                    <ThemedText style={styles.qaMeta}>
                      {q.asker_first_name} {q.asker_last_name}
                    </ThemedText>
                  </View>
